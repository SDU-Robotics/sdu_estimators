...
* C++ source files:
  * `include/sdu_estimators/sdu_estimators.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/sdu_estimators.cpp` is the main file that implements this library.
  * `app/sdu_estimators_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/sdu_estimators_t.cpp` contains the unit tests for the library.
  ...
  * The `python/sdu_estimators` directory contains a Python
    package for the project. It contains a compiled Python module `_sdu_estimators`
    that CMake generates from the Pybind11 source file `python/sdu_estimators/_sdu_estimators.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
